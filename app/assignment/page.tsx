...
// Aquí defines los mismos navItems que en el Dashboard
const navItems = [
  { icon: LayoutDashboard, label: "día, semana, mes, año", href: "/dashboard" }, // Cambiar href a la ruta correspondiente
...
export default function AssignmentPage() {
    ...
    const handleUpload = async () => {
      ...
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/upload_mes/`, {
          method: 'POST',
          body: formData,
        });
    ...
    const handleGenerate = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/generate_monthly_indicators/`, {
          method: 'POST',
          body: formData,
        });
        ...
        <div className={`flex-1 flex flex-col overflow-hidden transition-all duration-300 ease-in-out ${sidebarOpen ? 'ml-64' : 'ml-16'}`}>
          ...
          <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 dark:bg-gray-900">
            <div className="container mx-auto px-6 py-8">
              <h3 className="mt-8 text-3xl font-medium text-gray-700 dark:text-gray-200">Asignaciones y Cierre de Órdenes</h3>
